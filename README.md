...
### \_autoPlay (boolean):
If set to `true`, the video will autoplay. Will behave differently on mobile devices where user interaction may be reqired to start play.
...
### \_showCaptionsButton (boolean):
If set to `true`, video will show a button to control the Closed Captions. The default is `false`.  
...
### \_showReplayOverlay (boolean):
When enabled, the play button will be replaced with a replay button after the video has completed.
...
### \_closedCaptionsIcon (string):
CSS class name for the icon to be shown on the Closed Captions button.

### closedCaptionsNoneButton (string):
This text appears on the button that set the Closed Caption to none.
...
### inlineTranscriptTitle (string):
This optional text appears as the notify title for the transcript.
